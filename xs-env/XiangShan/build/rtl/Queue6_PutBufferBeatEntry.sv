...
module Queue6_PutBufferBeatEntry(
  input          clock,
  input          reset,
  input          io_enq_valid,
  ...
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      ...
      if (do_enq) begin
        if (enq_ptr_value == 3'h5)
          enq_ptr_value <= 3'h0;
        else
          enq_ptr_value <= 3'(enq_ptr_value + 3'h1);
      end
      if (do_deq) begin
        if (deq_ptr_value == 3'h5)
          deq_ptr_value <= 3'h0;
        else
  ...
  ram_6x288 ram_ext (
    ...
    .W0_en   (do_enq),
    .W0_clk  (clock),
    .W0_data ({io_enq_bits_mask, io_enq_bits_data})
  );
  assign io_deq_bits_data = _ram_ext_R0_data[255:0];
  assign io_deq_bits_mask = _ram_ext_R0_data[287:256];
endmodule
